...
def calculate_bollinger_bands(df, n=20, std=2):
    reversed_closes = df['Close'][::-1]
...
def calculate_price_change(current_date, df2):
    next_week_date = current_date + timedelta(days=28)
...
merged_df.to_csv('Backtest_natural_gas.csv', index=False)

merged_df = merged_df.dropna(subset=['PriceDifference'])
# Save or use the updated DataFrame as needed
merged_df.to_csv('Train_natural_gas.csv', index=False)
